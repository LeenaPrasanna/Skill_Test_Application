 ...
 $stm->execute();
  $stmr=$stm->get_result();
   $data=$stmr->fetch_assoc();
    $stmi = $con->prepare("insert into hardresult(user_id) values(?) ");
   $stmi->bind_param("i",$data['id']);
 $stmi->execute();
 $stms=$con->prepare("select score from hardresult where user_id=".$data['id']);
$stms->execute();
...
if($value['score']>$_SESSION['result']){
 ...
 $stmu = $con->prepare("update hardresult set score = ? WHERE user_id=? ");
