...
<html>
<head>
	<title> MEDIUM LEVEL QUESTIONS</title>
...
echo "<h3> LEVEL : MEDIUM</h3>";
	...
	$page_first_result=($page-1)*$rpp;
			$stm=$con->prepare("select * from mediumquiz");
  $stm->execute();
  $stmr=$stm->get_result();
     $r=$stmr->num_rows;
		$stm=$con->prepare("select * from mediumquiz limit ".$page_first_result.','.$rpp);
  $stm->execute();
  $stmr=$stm->get_result();
...
$_SESSION['no_of_unattempted']=$unattempt[1];
   ...
   if($page==$r){
    ...
    header("location:mediumlevelresult.php");
  }
   }
   ...
   if($page>=2){
   	echo " <a href='mediumquestions.php?page=".($page-1)."'><button class='but'>previous</button></a>";
   }
   echo '       ';
   if($page<$r){
   	echo " <a  href='mediumquestions.php?page=".($page+1)."'><button class='but'>next</button></a>";
   }
   echo'</div>';
}
